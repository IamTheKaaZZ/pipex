...
/*   Updated: 2021/07/12 15:32:31 by bcosters         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**	Send the data from the previous command to the next command
**	1.	REDIRECT the READ_END of pipe1 to STDIN
**	2.	CLOSE the pipe
**	3.	REDIRECT the WRITE_END of pipe2 to STDOUT
**	4.	CLOSE the pipe
**	5.	CLOSE fd_out and fd_in (gets inherited through fork)
*/

void	pipe_cmd_to_cmd(t_pipex *p, int cmd_i)
{
	dup2(p->pipe[READ_END], STDIN_FILENO);
	close_pipe(p->pipe);
